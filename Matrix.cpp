...
Vector::Vector(float r, float g, float b, float a)
{
	x = r;
	y = g;
	z = b;
...
Matrix Matrix::returnRotate(float degrees, float vx, float vy, float vz) {
	Matrix mat;
...
void Matrix::storeInverse(Matrix &mt) {
	mt = m;
	mt.m[12] = -1 * mt.m[12]/mt.m[0];
	mt.m[13] = -1 * mt.m[13]/mt.m[5];
	mt.m[14] = -1 * mt.m[14]/mt.m[10];
	mt.m[0] = 1 / mt.m[0];
	mt.m[5] = 1 / mt.m[5];
	mt.m[10] = 1 / mt.m[10];
}
